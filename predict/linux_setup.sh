...
function upload_csv {
sudo cp $1 /tmp/
mysql -u$USER $DB -e "LOAD DATA LOCAL INFILE \"/tmp/$1\" INTO TABLE $2 COLUMNS TERMINATED BY \",\" IGNORE 1 LINES"
sudo rm /tmp/$1
}
...
function clean_csv {
  #Added dec 2017
  dos2unix $1.csv
  CMCOUNT=$(head -n 1 $1.csv | sed 's/[^,]//g' | awk '{ print length }')
  if [ $CMCOUNT -gt 63 ]; then
	sed -i 's/,,,$//g' $1.csv
  fi
  #---------------------
  if [ "$1" == "EC" ]; then
...
$PWDIR/weight_table.py $1
}
function run_table {
mysql -u$USER $DBL -e "DROP TABLE IF EXISTS $1"
$PWDIR/run_table.py $1
}
