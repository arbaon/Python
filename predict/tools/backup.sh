...
mysql -u$USER $DB -e "create table $DBO (select Divn, Kickoff,HomeTeam,AwayTeam,if(B365D<3.3,\"D\",\"-\") as B365_D,if(B365H<B365A,if(BWH<B365D,\"H\",\"D\"),if(B365A<B365D,\"A\",\"D\")) as B365,if(BWD<3.3,\"D\",\"-\") as BW_D,if(BWH<BWA,if(BWH<BWD,\"H\",\"D\"),if(BWA<BWD,\"A\",\"D\")) as BW,if(IWD<=3.3,\"D\",\"-\") as IW_D,if(IWH<IWA,if(IWH<IWD,\"H\",\"D\"),if(IWA<IWD,\"A\",\"D\")) as IW,if(B365A<3.2,\"D\",\"-\") as LB_D,if(B365H<B365A,if(B365H<B365A,\"H\",\"D\"),if(B365A<B365A,\"A\",\"D\")) as LB,if(PSD<3.2,\"D\",\"-\") as PS_D,if(PSH<PSA,if(PSH<PSD,\"H\",\"D\"),if(PSA<PSD,\"A\",\"D\")) as PS,if(WHD<3.2,\"D\",\"-\") as WH_D,if(WHH<WHA,if(WHH<WHD,\"H\",\"D\"),if(WHA<WHD,\"A\",\"D\")) as WH,if(VCD<3.2,\"D\",\"-\") as VC_D,if(VCH<VCA,if(B365H<VCD,\"H\",\"D\"),if(VCA<VCD,\"A\",\"D\")) as VC,round((BWH+IWH+B365H+PSH+WHH+VCH)/6,2) as H,round((BWD+IWD+B365A+PSD+WHD+VCD)/6,2) as D,round((BWA+IWA+B365A+PSA+WHA+VCA)/7,2) as A from $2)"
mysql -u$USER $DB -e "drop table $2"
}
function create_fix_table {
mysql -u$USER $DB -e "DROP TABLE IF EXISTS $DBT"
mysql -u$USER $DB -e "create table $DBT (id int not null auto_increment,dvn varchar(10), KO date,h_team varchar(35), a_team varchar(35),lp int, fp int, lpf int, fpf int, wa int, wha int, fib int, gls int, alg int,tot int,result varchar(5),slip int,primary key (id))"
}
