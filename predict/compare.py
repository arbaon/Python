...
def get_goals(year,team,ha):
    ...
    return gls
def get_more_goals(year,team):
    G="Goals_f"
    GA="Goals_a"
    query="""select %s,%s from %s_play_table where team="%s" """ %(G,GA,year,team)
    results=run_query("Leagues",query,"Q")
    mgls=[float(results[0][0]),float(results[0][1])]
    return mgls
...
def init():
    ...
    TeamA = TeamA + get_more_goals(YR,TeamA[0])#more_goals
    TeamB = TeamB + get_more_goals(YR,TeamB[0])#more_goals
...
def predict(TeamA,TeamB,Top,dvn,ko,ID):
    ...
    MGLS=(TeamA[13]+TeamB[14])-(TeamB[13]+TeamA[14])
    ...
    tmph=uni_conv(GLS,4) # goals
    tmpi=uni_conv(MGLS,4) # goals

    tot=(tmpa+tmpc+tmpe+tmpg+tmpi)+((tmpb+tmpd+tmpf+tmph)*1.25)
    tmpj=uni_conv(tot,5)
    print tmpa,tmpb,tmpc,tmpd,tmpe,tmpf,tmpg,tmph,tmpi,tmpj,tot,MGLS
    if str(sys.argv[6]) == "y":
    	#query="insert into %s (dvn,KO,h_team,a_team,lp,fp,lpf,fpf,wa,wha,fib,gls,alg,tot) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % ("Predict",dvn,ko,TeamA[0],TeamB[0],tmpa,tmpb,tmpc,tmpd,tmpe,tmpf,tmpg,tmph,tmpi,tmpj)
    	#query="update %s set lp=%s,fp=%s,lpf=%s,fpf=%s,wa=%s,wha=%s,fib=%s,gls=%s,alg=%s,tot=%s where ID=%s" % ("predict",tmpa,tmpb,tmpc,tmpd,tmpe,tmpf,tmpg,tmph,tmpi,tmpj,ID)
    	query="update %s set lp=%s,fp=%s,lpf=%s,fpf=%s,wa=%s,wha=%s,fib=%s,gls=%s,alg=%s,tot=%s,goalpw=%s,totpw=%s  where ID=%s" % ("predict",tmpa,tmpb,tmpc,tmpd,tmpe,tmpf,tmpg,tmph,tmpi,tmpj,MGLS,tot,ID)
...
if total > 1:
    rw=[]
    rw.append([1.5,1.1,0.7,0.3]) #lp / lf
    rw.append([1.7,1.3,0.9,0.5]) #lp /fp
    rw.append([5.1,3.8,2.5,1.2])#wa / wha
    rw.append([2.8,2.1,1.4,0.7])# consis
    rw.append([2.0,1.5,1.0,0.5])# gls
    rw.append([21,15,9,3])# tots
    init()
else:
